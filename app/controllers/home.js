...
var redisClient = redis.createClient({host:'gutenberg-ir.redis.cache.windows.net', port: process.env.PORT, password: 'B4qWA879R/U2ldA3mWT5kcJSHrDXOijbd9ju+89PNhg=', db: 1});
        ...
        getTFIDFFromService(doc, wordQuery).then(function (tfidf) {
          ...
          if (wordWithTfidfMap.size === resNum) {
            ...
            getMagnitudeFromService(doc).then(function (magnitude) {
              ...
              console.log("Promise getMagnitudeFromService Rejected");
              console.error(error);
            });
          }
        }).catch(function (error) {
          console.log("Promise getTFIDFFromService Rejected");
          console.error(error);
        });
      });
    });
